...
function getConfigPath(CWD, testFileDir, configFromUserInput) {
  ...
  return (
    findUp.sync("malaby-config.json", { cwd: testFileDir }) ||
    findUp.sync("malaby-config.js", { cwd: testFileDir })
  );
}
...
const buildCommandString = (config, filePath, options) => {
  ...
  const commandBinCliPath = path.join(
    cwd,
    "node_modules",
    ".bin",
    npmModuleCmd
  ...
  if (
    fs.existsSync(commandBinCliPath) &&
    (inspectPort || isDebug || isInspect)
  ) {
  ...
  if (inspectPort) {
    cmd = cmd.replace(
      "node ",
      `node --inspect-brk=${Number(inspectPort) + 1} `
    ); // Hail to Webstorm
...
const getFilesToWatch = (context) =>
  _(DEFAULT_FILES_TO_WATCH)
...
const getFilesToIgnore = (context) =>
  _.map(context.config.ignoreOnWatch, globToRegExp);
